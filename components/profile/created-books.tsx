  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 w-full">
      {projects.map((project) => (
        <Card key={project.id} className="overflow-hidden flex flex-col w-full h-full">
          <div className="aspect-square relative flex-shrink-0">
            <Image src={project.coverImage || "/placeholder.svg"} alt={project.title} layout="fill" objectFit="cover" />
          </div>
          <CardContent className="p-3 sm:p-4 flex flex-col flex-grow relative">
            <div className="flex flex-wrap items-center mb-2">
              <h3 className="font-semibold text-base sm:text-lg text-power-pump-heading mr-2 mb-1">{project.title}</h3>
            ...
            <div className="mt-auto">
              <p className="text-power-pump-text mb-2">Price: {project.price} SOL</p>
              <div className="mb-2">
                <Progress
              ...
              <div className="flex flex-col text-sm text-power-pump-subtext">
                ...
                <span>{(project.mintedSupply * project.price).toFixed(2)} SOL collected</span>
              </div>
            </div>
            <button
              onClick={() => window.open(`https://explorer.solana.com/address/${project.id}`, "_blank")}
