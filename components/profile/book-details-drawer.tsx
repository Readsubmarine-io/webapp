...
import { Download, DollarSign, XCircle } from "lucide-react"
...
export function BookDetailsDrawer({ bookId, isOpen, setIsOpen, isOnSale = false }: BookDetailsDrawerProps) {
      ...
      <SheetContent className="w-screen sm:w-[450px] sm:max-w-[450px] flex flex-col bg-drawer">
        <div className="flex-grow overflow-y-auto pb-4">
          <SheetHeader className="mb-4">
            <div className="flex flex-col">
              <div className="flex items-center justify-between">
                <SheetTitle className="text-2xl font-bold text-power-pump-heading">{book.title}</SheetTitle>
              </div>
              <SheetDescription className="text-power-pump-text mt-1 text-left">by {book.author.name}</SheetDescription>
            </div>
          </SheetHeader>
            ...
            <div className="flex flex-col justify-between">
              <div className="space-y-2">
                <p className="text-sm text-power-pump-text">
                  Token ID: <span className="font-semibold">#{book.tokenId}</span>
                </p>
                <p className="text-sm text-power-pump-text">
                  Purchase Price: <span className="font-semibold">{book.purchasePrice} SOL</span>
                </p>
                {isOnSale && <ListPrice price={listPrice} onPriceChange={(newPrice) => setListPrice(newPrice)} />}
                <p className="text-sm text-power-pump-text">
                  Floor Price: <span className="font-semibold">{book.floorPrice} SOL</span>
          ...
          <div className="space-y-4">
            <div>
              <h3 className="text-lg font-semibold text-power-pump-heading mb-2">Book Details</h3>
              <div className="text-sm space-y-2">
                <p className="text-power-pump-text">
                  Format: <span className="font-semibold">{book.details?.format}</span>
                </p>
                <p className="text-power-pump-text">
                  Pages: <span className="font-semibold">{book.details?.pages}</span>
                </p>
                ...
                <p className="text-power-pump-text">
                  Published: <span className="font-semibold">{book.details?.publishedDate}</span>
                </p>
              </div>
            </div>
            <Separator className="my-4" />
            <div>
              <h3 className="text-lg font-semibold text-power-pump-heading mb-2">About the Author</h3>
              <div className="flex items-center gap-4 mb-4">
                ...
                />
                <div>
                  <h4 className="font-semibold text-power-pump-heading">{book.author.name}</h4>
                  <p className="text-sm text-power-pump-text">{book.author.title}</p>
                </div>
              </div>
            ...
            <Separator className="my-4" />
            <div>
              <h3 className="text-lg font-semibold text-power-pump-heading mb-2">About this book</h3>
              <p className="text-sm text-power-pump-text">{book.description || "No description available."}</p>
            </div>
          </div>
        </div>
        <div className="mt-auto pt-4 border-t border-gray-200">
          <div className="flex justify-between gap-4">
            ...
            <Button
              className={`flex-1 ${
                isOnSale ? "bg-red-600 hover:bg-red-700" : "bg-green-600 hover:bg-green-700"
              } text-white`}
