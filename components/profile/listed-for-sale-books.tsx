...
import { MoreVertical, Eye, XCircle, Download } from "lucide-react"
...
interface ListedForSaleBooksProps {
  books: Book[]
}

export function ListedForSaleBooks({ books }: ListedForSaleBooksProps) {
        ...
        {books.map((book) => (
          <Card key={book.id} className="overflow-hidden h-full relative flex flex-row">
            ...
            <CardContent className="p-4 flex flex-col w-2/3 relative">
              <div className="flex justify-between items-start mb-2">
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent className="w-48 sm:w-56 bg-contextMenu text-contextMenu-foreground">
                    ...
                    <DropdownMenuItem
                      className="cursor-pointer flex items-center"
                      onSelect={() => console.log("Cancel Sale", book.id)}
                    >
                      <XCircle className="mr-2 h-3 w-3 sm:h-4 sm:w-4" />
                      <span className="text-sm">Cancel Sale</span>
                    </DropdownMenuItem>
                    <DropdownMenuItem
                      className="cursor-pointer flex items-center"
              ...
              <div className="flex flex-col space-y-1">
                ...
                <ListPrice
                  ...
                  onPriceChange={(newPrice) => {
                    // Update the book's list price
                    console.log(`Updated price for book ${book.id}: ${newPrice} SOL`)
                  }}
                />
                <span className="text-sm text-power-pump-text">
              ...
              <Badge className="bg-power-pump-button text-white mt-4 self-start">On Sale</Badge>
            </CardContent>
          </Card>
        ))}
      </div>
