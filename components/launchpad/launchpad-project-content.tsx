...
export function LaunchpadProjectContent({ project }: LaunchpadProjectContentProps) {
      ...
      <div className="flex flex-col md:flex-row gap-8">
        <section className="md:w-1/3">
          <figure>
            ...
            />
            <figcaption className="sr-only">Book cover for {project.title}</figcaption>
          </figure>
        </section>

        <section className="md:w-2/3">
          <header className="mb-8">
            <h1 className="text-3xl font-bold mb-2 text-power-pump-heading">{project.title}</h1>
            ...
            <CardContent className="p-4">
              <div className="mb-4">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span className="text-power-pump-heading font-semibold">
                    {project.mintedSupply} / {project.totalSupply} [{progressPercentage.toFixed(0)}%]
                  </span>
                </div>
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <p className="font-semibold text-power-pump-heading">{project.backers}</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="font-semibold text-power-pump-heading">{daysLeft} days</p>
                </div>
                <div>
          ...
          <MintingSection price={project.price.amount} currency={project.price.currency} />

          <div className="grid lg:grid-cols-2 gap-6 mb-6">
            <Card className="border border-container-border rounded-xl shadow-content-container">
              <CardContent className="p-4">
                <h2 className="text-xl font-bold text-power-pump-heading mb-4">Book Details</h2>
                <div className="space-y-4 text-sm">
                  <div className="flex items-center gap-2">
                    <span className="text-power-pump-text">Format:</span>
                    <span className="font-medium text-power-pump-text">eBook</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="text-power-pump-text">Pages:</span>
                    <span className="font-medium text-power-pump-text">{project.details.pages}</span>
                  </div>
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-medium text-power-pump-text">{project.details.publishedDate}</span>
                    </div>
                  </div>
                </div>
              </CardContent>
            ...
            <Card className="border border-container-border rounded-xl shadow-content-container">
              <CardContent className="p-4">
                <h2 className="text-xl font-bold text-power-pump-heading mb-4">About the Author</h2>
                <div className="flex items-center gap-4 mb-4">
                  ...
                  />
                  <div>
                    <h3 className="font-semibold text-power-pump-heading">{project.author.name}</h3>
                    <p className="text-sm text-power-pump-text">{project.author.title}</p>
                  </div>
                </div>
                <p className="text-sm text-power-pump-text leading-relaxed">{project.author.bio}</p>
              </CardContent>
            </Card>
          </div>
