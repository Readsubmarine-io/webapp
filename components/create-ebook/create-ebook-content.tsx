...
const steps = ["Initial Information", "Additional Details", "Minting Details", "File Uploads"]
...
export function CreateEbookContent() {
  ...
  const renderStep = () => {
    switch (currentStep) {
      case 0:
        return <InitialInformation formData={formData} updateFormData={updateFormData} onNext={nextStep} />
      case 1:
        return (
          <AdditionalDetails formData={formData} updateFormData={updateFormData} onNext={nextStep} onPrev={prevStep} />
        )
      case 2:
        return (
          <MintingDetails formData={formData} updateFormData={updateFormData} onNext={nextStep} onPrev={prevStep} />
        )
      case 3:
        return <FileUploads formData={formData} updateFormData={updateFormData} onPrev={prevStep} />
      default:
        return null
    }
  }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <h1 className="text-3xl font-bold text-power-pump-heading mb-6">Create Your NFT eBook</h1>
      <div className="mb-8">
            ...
            >
              <span
                className={`flex items-center justify-center w-6 h-6 mr-2 text-xs rounded-full shrink-0 transition-all ${
                  index <= currentStep ? "bg-power-pump-button text-white" : "bg-gray-200 text-gray-600"
                }`}
              >
                {index + 1}
              </span>
