...
export function MintingDetails({ formData, updateFormData, onNext, onPrev }: MintingDetailsProps) {
  ...
  const validate = () => {
    ...
    if (!formData.mintPrice.trim()) {
      ...
      isValid = false
    } else if (isNaN(Number(formData.mintPrice)) || Number(formData.mintPrice) < 0) {
    ...
    if (!formData.totalCopies.trim()) {
      ...
      isValid = false
    } else if (isNaN(Number(formData.totalCopies)) || Number(formData.totalCopies) <= 0) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <div>
        ...
        />
        {errors.mintPrice && <p className="text-red-500 text-sm mt-1">{errors.mintPrice}</p>}
        <p className="text-sm text-gray-500 mt-1">
          Set to 0 if you want to offer the eBook for free. Royalties are fixed at 5% (4% to the writer, 1% to the
          platform).
        </p>
      </div>
      <div>
      ...
      <div>
        ...
        />
        {errors.mintDate && <p className="text-red-500 text-sm mt-1">{errors.mintDate}</p>}
      </div>
      <div>
        ...
        />
        {errors.mintTime && <p className="text-red-500 text-sm mt-1">{errors.mintTime}</p>}
        <p className="text-sm text-gray-500 mt-1">Mints remain open for seven days or until all NFTs are minted.</p>
      </div>
