    ...
    <div className="relative w-full pt-4 sm:pt-6 pb-6 sm:pb-8">
      ...
      >
        {collections.map((collection) => (
          <div key={collection.id} className="snap-start shrink-0 pr-4 sm:pr-6 w-[280px]">
            <CollectionCard collection={collection} />
          </div>
        ))}
      </div>
      {showLeftArrow && <SliderButton direction="left" onClick={() => scroll("left")} />}
      {showRightArrow && <SliderButton direction="right" onClick={() => scroll("right")} />}
    </div>
  )
}
...
function CollectionCard({ collection }: { collection: Collection }) {
  ...
  return (
    <Link href={`/marketplace/${collection.id}`}>
      ...
      >
        <Image src={collection.image || "/placeholder.svg"} alt={collection.title} layout="fill" objectFit="cover" />
